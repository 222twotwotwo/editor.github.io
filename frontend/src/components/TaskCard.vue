<template>
  <div class="task-card" :class="[`priority-${task.priority}`, { 'is-completed': task.status === 'completed', 'is-overdue': isOverdue }]">
    ...
    <div v-if="task.due_date" class="task-due-date" :class="{ 'overdue': isOverdue, 'due-today': isDueToday }">
...
const statusToggleTitle = computed(() => {
  switch (props.task.status) {
    case 'pending': return '点击开始进行'
    case 'in_progress': return '点击标记完成'
    case 'completed': return '点击重置为待处理'
    default: return '切换状态'
  }
})
...
const isOverdue = computed(() => {
  ...
  const taskDate = getDateOnly(props.task.due_date)
...
const formatDueDate = (dateStr) => {
  ...
  const taskDate = getDateOnly(dateStr)

  if (taskDate.getTime() === today.getTime()) return '今天截止'
  if (taskDate.getTime() === tomorrow.getTime()) return '明天截止'
